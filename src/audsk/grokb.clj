...
(defn draw-proxy []
  (if @paused?
	...
	(d-path c))
      ...
      ;; ;; This draws a sinuous clock-hand
      ;; (with-translation [(half *screen-width*) (half *screen-height*)]
      ;; 	(with-rotation [ (mod (/ @*time* 10) TWO_PI) ]
      ;; 	  ;; (stroke 0 0 0 255)
      ;; 	  ;; (stroke-weight    2)
      ;; 	  (no-stroke)	  
      ...
      ;; 	      (let [x (* 15 (sin (- (->new-range r [lo hi] [0 (* 1 TWO_PI)])
      ;; 				   PI)))
...
;; .. include a watcher that recomputes *id-mapping*'s inverse on changes
...
;;
;; (with-tuio [t tobj]
;;   (swap! *cubes* merge-with into {(:id t) t}))
;;;;
...
(defn rem-tuio-object [tobj]
  (let [fid (.getSymbolID tobj)
	...
	rot-speed (.getRotationSpeed tobj)
	rot-accel (.getRotationAccel tobj)]
    (swap! *cubes* merge-with into {fid {:id fid
					 :pos [x y]
					 :speed mov-speed
					 :accel mov-accel
					 :angle angle
